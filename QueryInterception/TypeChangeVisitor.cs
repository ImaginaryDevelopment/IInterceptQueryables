		...
		private NewExpression TransformNewCall(NewExpression node)
		{
			...
			if ((!constructor.DeclaringType.IsGenericType ? false : constructor.DeclaringType.GetGenericArguments().Any<Type>(new Func<Type, bool>(this.NeedsTypeChange))))
			{
				...
				Type[] constructorTypes = (
					from s in (IEnumerable<ParameterInfo>)constructor.GetParameters()
			...
			IEnumerable<MemberInfo> members = memberInfos.Join<MemberInfo, MemberInfo, string, MemberInfo>((IEnumerable<MemberInfo>)memberInfoArray, name, func, (MemberInfo fMember, MemberInfo nMember) => nMember);
